...
function checkFilter(xpath, elementType) {
  ...
  var elements = Aliases.browser.pageMagayaDigitalFreightPortalHu.FindElements(xpath);
  var elementCount = elements ? elements.length : null;
 
  // Extract the text from the 'p-datatable-footer' class
  var footerText = Aliases.browser.pageMagayaDigitalFreightPortalHu.FindElement("//div[contains(@class, 'p-datatable-footer')]").textContent;
...
function checkShipmentReference() {
  var shipmentXPath = "//a[contains(@href, '/shipments/') and normalize-space(text())='SRTM00019742']";
  checkFilter(shipmentXPath, "Shipment Reference SRTM00019742");
}
 
function checkCustomer() {
  var shipmentXPath = "//div[normalize-space(text())='Customer 4471']";
  checkFilter(shipmentXPath, "Customer 4471");
}
 
function checkCustomerReference() {
  var shipmentXPath = "//div[@class='small' and contains(normalize-space(text()), 'Ocean FCL Los Angeles to Shanghai')]";
  checkFilter(shipmentXPath, "Customer Reference Ocean FCL Los Angeles to Shanghai");
}
...
function checkMasterBL() {
  var completedXPath = "//div[normalize-space(text())='HLCUSEL240157506']";
  checkFilter(completedXPath, "MasterBL HLCUSEL240157506");
}
 
function checkHouseBL() {
  var completedXPath = "//div[normalize-space(text())='TEST_3009_1']";
  checkFilter(completedXPath, "HouseBL TEST_3009_");
}
 
function checkETD() {
  var completedXPath = "//span[contains(normalize-space(text()), '01/06/2025')]";
  checkFilter(completedXPath, "ETD 01/06/2025");
}
function checkETA() {
  var completedXPath = "//span[contains(normalize-space(text()), '01/06/2025')]";
  checkFilter(completedXPath, "ETA 01/06/2025");
}
